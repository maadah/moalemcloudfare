// ─────────────────────────────────────────────────────────────────────────────
// geminiService.ts — now powered by Kimi K2.5 via OpenRouter
//
// Despite the filename, this module uses Kimi K2.5 (moonshotai/kimi-k2.5)
// through OpenRouter's unified gateway. The file name and exported function
// names are kept identical so the rest of the project doesn't need changes.
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// API key helpers — dual-key fallback for OpenRouter
//
// Cloudflare Pages environment variables (Settings → Environment Variables):
//   VITE_OPENROUTER_API_KEY            ← primary key (sk-or-v1-...)
//   VITE_OPENROUTER_API_KEY_SECONDARY  ← backup key  (sk-or-v1-...)
//   VITE_OPENROUTER_MODEL              ← optional, defaults to moonshotai/kimi-k2.5
//
...
const LS_PRIMARY = "OPENROUTER_API_KEY_FALLBACK";
const LS_BACKUP  = "OPENROUTER_API_KEY_BACKUP";

const OPENROUTER_BASE_URL = "https://openrouter.ai/api/v1";

// Model identifier on OpenRouter — reads from env var, defaults to kimi-k2.5
const OPENROUTER_MODEL = (() => {
  try {
    const m = (import.meta.env as Record<string, string>)["VITE_OPENROUTER_MODEL"];
...
const getPrimaryKey = (): string => readKey("VITE_OPENROUTER_API_KEY",           LS_PRIMARY);
const getBackupKey  = (): string =>
  readKey("VITE_OPENROUTER_API_KEY_SECONDARY", LS_BACKUP) ||
  readKey("VITE_OPENROUTER_API_KEY_2",         LS_BACKUP);
...
const getApiKeyErrorMessage = (): string => {
  ...
  if (isCloudflare) {
    return "مفتاح OpenRouter API غير مضبوط. تأكد من إضافة المتغيرات في Cloudflare Pages → Settings → Environment Variables باسم VITE_OPENROUTER_API_KEY ثم أعد نشر المشروع (Redeploy). يمكنك أيضاً إدخال المفتاح يدوياً من أيقونة الإعدادات (⚙️).";
  }
  return "مفتاح OpenRouter API غير مضبوط. يرجى الضغط على أيقونة الترس (⚙️) في الأعلى وإدخال مفتاح OpenRouter للمتابعة.";
};

// ─────────────────────────────────────────────────────────────────────────────
// OpenRouter API request — uses OpenAI-compatible chat/completions endpoint
// ─────────────────────────────────────────────────────────────────────────────
...
async function callKimi(apiKey: string, opts: KimiCallOptions): Promise<string> {
  ...
  const res = await fetch(`${OPENROUTER_BASE_URL}/chat/completions`, {
    ...
    headers: {
      ...
      // OpenRouter recommends these headers for app identification & free-tier eligibility
      "HTTP-Referer": typeof window !== "undefined" ? window.location.origin : "",
      "X-Title": "Moalem - Smart Grader",
    },
...
async function withKeyFallback<T>(
  ...
  try {
    ...
    console.warn(
      "[openrouter] Primary key failed, switching to backup.",
      primaryError?.message?.slice(0, 80)
    );
    try {
      return await fn(backup, "backup");
    } catch (backupError: any) {
      console.error("[openrouter] Backup key also failed.");
      throw backupError;
    }
  }
}
