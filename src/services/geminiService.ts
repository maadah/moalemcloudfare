...
// ── API keys & model from environment variables (Cloudflare) ────────────────
// All keys are read at BUILD time (Vite inlines VITE_* vars). We try them in
// order; if one fails (quota/invalid/503/429), we fall back to the next.
...
const getApiKeyErrorMessage = () =>
  'مفتاح API غير مضبوط. يرجى إضافة المفاتيح (VITE_GEMINI_API_KEY) في إعدادات البيئة (Environment Variables) في Cloudflare ثم إعادة النشر.';

// Whether an error is worth retrying with a DIFFERENT key.
const isRetryableKeyError = (err: any): boolean => {
  const msg = String(err?.message || err || '').toLowerCase();
  return (
    msg.includes('api key') || msg.includes('api_key') || msg.includes('permission') ||
    msg.includes('quota') || msg.includes('resource_exhausted') ||
    msg.includes('429') || msg.includes('503') || msg.includes('overloaded') || msg.includes('unavailable')
  );
};
...
export async function gradeStudentPaper(
  ...
  try {
    ...
    1. LOCATE the student's final result = THE LAST VALUE THE STUDENT REACHED in their whole solution.
       Think of it as: follow the student's pen to where the work ENDS, and take the value it stopped at.
       - The solution may span MORE THAN ONE LINE; if the student moved to a new (lower) line to continue,
         the final result is on that LAST line they wrote.
       - Do NOT worry about left vs right or Arabic vs English direction. Just take the LAST value the
         student arrived at — the isolated value with NO further operation after it.
    ...
    const response = await generateWithFallback({
      ...
      config: { 
        ...
        temperature: 0,
        systemInstruction:
          "أنت معلم عراقي خبير وروبوت استخراج نصوص حرفي، تصحّح كل المواد. أولاً حدّد نوع كل سؤال: رقمي (رياضيات/مسائل حسابية) أو نصي (تعريف/شرح/قانون/قواعد). 1) الاستخراج: اكتب ما تراه في خط الطالب بدقة 100% حتى لو كان خطأً، واحتفظ بنظام أرقامه (عربي يبقى عربي). إذا رأيت '68-' اكتب '68-' ولا تصححها، ولا تنسخ الإجابة النموذجية أبداً، والسؤال الفارغ تكتبه فارغاً ودرجته صفر. 2) التصحيح: للأسئلة الرقمية ركّز فقط على الناتج النهائي وهو آخر قيمة وصل إليها الطالب في حلّه (تتبّع القلم حتى نهاية الحل، وقد يكون في سطر جديد نزل إليه، بغض النظر عن اليمين أو اليسار)؛ حوّله إلى كلمات (مثل ٢٥ تصبح خمسة وعشرون، و-١٥ تصبح سالب خمسة عشر) واقرأ كل خانة بترتيبها دون قلب، ثم حوّل آخر رقم في الإجابة النموذجية إلى كلمات وقارن: إن تطابقا فالدرجة كاملة وإلا صفر، بلا درجة جزئية في الرياضيات. لا تحلّل الخطوات الوسطى. للأسئلة النصية قارن المعنى والمفاهيم بالنموذج واقبل إعادة الصياغة وامنح درجة جزئية إن غطّى بعض النقاط. اجعل الملاحظات (feedback) بالعربية الفصحى وبأسلوب تربوي."
      }
    });
