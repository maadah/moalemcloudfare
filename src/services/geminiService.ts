...
export async function gradeStudentPaper(
  ...
  try {
    ...
    // ═══════════════════════════════════════════════════════════
    const readingPrompt = `أنت آلة تصوير ذكية. مهمتك الوحيدة: نسخ ما هو مكتوب في الصورة.
...
إذا رأيت رقماً يبدو خاطئاً رياضياً → انقله كما هو، لأن هذا ما كتبه الطالب فعلاً.
إذا رأيت إشارة تبدو مقلوبة → انقلها كما هي.
إذا رأيت ناتجاً يختلف عن الخطوات → انقل الناتج كما هو.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
قاعدة الناتج النهائي
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
الكتابة العربية من اليمين لليسار.
الناتج النهائي = آخر رقم يظهر بعد آخر علامة = في المعادلة كاملة.
إذا امتدت المعادلة على سطرين → الناتج في بداية السطر الثاني (أقصى اليسار).
اتبع سلسلة = من البداية حتى نهايتها — لا تقف عند أول رقم.
    ...
    try {
      const readingResponse = await generateWithGeminiFallback({
        ...
        config: {
          responseMimeType: "application/json",
          temperature: 0,
          systemInstruction: `أنت آلة تصوير ذكية — تنسخ ما في الصورة فقط.

المبدأ الأساسي:
ما تراه في الصورة هو الحقيقة الوحيدة بالنسبة لك.
لا توجد إجابة صحيحة أو خاطئة بالنسبة لك — فقط ما هو مكتوب.
أي رقم أو إشارة أو ناتج تراه → انقله كما هو بدون تغيير.

قاعدة الناتج النهائي:
اتبع سلسلة = من البداية حتى آخر رقم في المعادلة.
