...
// Extract questions from a single set of exam paper images.
...
export const onRequestPost = async (context: Ctx): Promise<Response> => {
  try {
    ...
    if (!base64Images || !Array.isArray(base64Images)) {
      return jsonResponse({ error: "الرجاء رفع صور ورقة الأسئلة للمتابعة." }, 400);
    }
    ...
    const parts: any[] = base64Images.map((data: string) => {
    ...
    return jsonResponse({ error: error.message || "حدث خطأ أثناء استخراج بيانات ورقة الامتحان." }, 500);
  }
};
